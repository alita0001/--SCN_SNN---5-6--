...
if __name__ == '__main__':
    ...
    Lmax = 50  # the maximum number of hidden nodes
    ...
    tol = 1e-2  # the tolerance error
    r = 0.7
    Lambdas = [0.1, 0.5, 1, 5, 10, 20, 50, 100]  # scope sequence
    ...
    total_test_acc = 0
    total_test_time = 0
    np.random.seed(6)  # 6 best
    # np.random.seed(8)  # 6 best
    for i in range(num_runs):
        # 加载iris数据集
        train_data, train_label, test_data, test_label = image_process_copy.load_and_preprocess_iris(test_size=0.2, shuffle=True, normalize=False, cross_validation=True, n_splits=5, fold_idx=i)

        train_acc, train_time, test_acc, test_time = SCN_classification_SNN(train_data, train_label, test_data, test_label, dl, delta, c, Lambdas, r, verbose, Lmax, tol,
                       Tmax, time_steps=500)
