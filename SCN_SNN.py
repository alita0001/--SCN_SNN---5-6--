...
#             normalized_data[:, i] = (feature - min_val) / (max_val - min_val)
#         else:
#             normalized_data[:, i] = 0.5  # 如果特征值都相同，设为0.5
...
def SCN_classification_SNN(train_x, train_y, test_x, test_y, dl, delta, c, Lambdas, r, verbose, Lmax, tol, Tmax,
    ...
    # # 对拼接后的数据进行泊松编码
    # data_x_spikes = poisson_encoding(data_x, time_steps)

    # # 将数据分割回训练和测试集
    # train_x_spikes = data_x_spikes[:,:train_x.shape[0]]   
    # test_x_spikes = data_x_spikes[:,train_x.shape[0]:]
    ...
    # 使用tqdm创建进度条
    from tqdm import tqdm
    pbar = tqdm(total=Lmax, desc="训练进度")
    ...
    while L < Lmax and Error > tol:
        ...
        # 更新进度条
        pbar.update(delta)
        pbar.set_postfix({'节点数': L, '训练准确率': f'{Error:.4f}', '测试准确率': f'{test_acc:.4f}'})
...
if __name__ == '__main__':
    ...
    # # 加载iris数据集
    # train_data, train_label, test_data, test_label = image_process_copy.load_and_preprocess_liver_disorders(test_size=0.2, shuffle=True, normalize=False)
    ...
    # np.random.seed(6)  # 每次迭代重置相同的种子 95.79%
    # np.random.seed(1)  # 每次迭代重置相同的种子 96.67%
    # np.random.seed(2)  # 每次迭代重置相同的种子 96.84%
    # np.random.seed(3)  # 每次迭代重置相同的种子 96.67%
    # np.random.seed(4)  # 每次迭代重置相同的种子 96.84%
    # np.random.seed(5)  # 每次迭代重置相同的种子 %
    # np.random.seed(6)  # 每次迭代重置相同的种子 %
    # np.random.seed(7)  # 每次迭代重置相同的种子 %
    # np.random.seed(8)  # 每次迭代重置相同的种子 %
    # np.random.seed(7)  # 每次迭代重置相同的种子 97.37%
    best_seed = 0
    best_acc = 0
    acc_list = []
