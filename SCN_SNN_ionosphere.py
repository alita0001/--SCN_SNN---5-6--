...
if __name__ == '__main__':
    ...
    # parameter setting
    train_data, train_label, test_data, test_label = image_process_copy.load_and_preprocess_ionosphere(test_size=0.1, shuffle=True, normalize=False)
    
    dl = 0  # with direct links
    Lambda = 1  # the assignment scope of random parameters
    Lmax = 45  # the maximum number of hidden nodes
    #######################################################
    delta = 1  # number of hidden node addition each step
    Tmax = 5
    tol = 1e-2  # the tolerance error
    ...
    for i in range(num_runs):
        ...
        # 绘制权重参数分布
        plt.figure(figsize=(10, 6))
        plt.hist(W_opt.flatten(), bins=100, edgecolor='black')
    ...
    # 将当前种子的所有模型保存在一个文件中
    model_file = os.path.join(model_dir, f'ionosphere_models.pkl')
