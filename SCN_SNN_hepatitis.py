...
def poisson_encoding(data, timesteps):
    ...
    for i in range(data.shape[1]):
        ...
        if col_max > col_min:  # 避免除以零
            ...
            normalized_data[:, i] = 0  # 如果所有值相同，则设为0
...
if __name__ == '__main__':
    ...
    Lmax = 5  # the maximum number of hidden nodes
    #######################################################
    delta = 5  # number of hidden node addition each step
    Tmax = 5
    tol = 1e-2  # the tolerance error
    r = 0.7
    ...
    # 将当前种子的所有模型保存在一个文件中
    model_file = os.path.join(model_dir, f'hepatitis_models.pkl')
